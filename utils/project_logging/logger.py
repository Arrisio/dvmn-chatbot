...
def configure_logger(
    ...
    """
    Процедура конфигурирует логгер Loguru, в зависимости от среды эксплуатации логгера

        :param set_default_prod_configuration: Автоматически применяются настройки для боевого режима: Вывод в json формат; ERROR и CRITICAL логи выводятся в stderr. Если True - Все остальные параметры (кроме level) игнорируются
...
logger.configure(**_get_loguru_config())  # если нас устраивают дефолтовые настройки и мы не хотим конфигурить логгер в коде
